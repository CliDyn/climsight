...
def retrieve_era5_data(
    ...
    try:
        logging.info(f"ERA5 retrieval: var={variable_id}, time={start_date} to {end_date}, "
        ...
        # CLI fallback: use environment-provided session id when available.
        if not main_dir:
            env_thread_id = os.environ.get("CLIMSIGHT_THREAD_ID")
            if env_thread_id:
                main_dir = os.path.join("tmp", "sandbox", env_thread_id)
                logging.info(f"Using CLI sandbox from CLIMSIGHT_THREAD_ID: {main_dir}")
