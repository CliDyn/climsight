...
def run_streamlit(config, api_key='', skip_llm_call=False, rag_activated=True, embedding_model='', chroma_path='', references=None):
    ...
    # RUN submit button - ANALYSIS LOGIC ONLY
    ...
    if 'last_output' in st.session_state and st.session_state['last_output']:
        ...
        if show_add_info_display and 'tab_add' in locals():
            with tab_add:
                stored_input_params = st.session_state['last_input_params']
                stored_figs = st.session_state['last_figs']
                
                st.subheader("Additional information", divider='rainbow')
                if 'lat' and 'lon' in stored_input_params:
        ...
        stored_input_params = st.session_state['last_input_params']
        stored_figs = st.session_state['last_figs']
        stored_references = st.session_state['last_references']
