    ...
    if submit_button and user_message:
        ...
        if use_era5_data:
            ...
            if not arraylake_api_key:
                ...
                st.stop()
            # Set as env var — NEVER store in config (LangSmith leaks it)
    ...
    if 'last_output' in st.session_state and st.session_state['last_output']:
        ...
        if show_add_info_display:
            with tab_figs:
                ...
                # Climate data plots are now generated by data_analysis_agent
                # Skip legacy plotting if analysis_images include climate plots
                analysis_images = stored_input_params.get('data_analysis_images', [])
                has_analysis_climate_plots = any('climate' in os.path.basename(p).lower() for p in analysis_images if os.path.exists(p))

                if not has_analysis_climate_plots:
                    # Fallback: Try to get climate data from unified 'climate_data' or 'high_res_climate' key
                    try:
                ...
                # Skip if analysis_images already include climate plots
                if not climate_data_displayed and not has_analysis_climate_plots and ('df_data' in data_pocket.df and data_pocket.df['df_data'] is not None):
                ...
                # Data analysis images (from predefined plots and python_repl)
                # All plots are now generated by data_analysis_agent and saved to results/
                analysis_images = stored_input_params.get('data_analysis_images', [])
                if analysis_images:
                    # Categorize images by type for better display
                    climate_plots = []
                    disaster_plots = []
                    population_plots = []
                    other_plots = []
                    ...
                    for image_path in analysis_images:
                        ...
                        if 'climate' in filename or 'temperature' in filename or 'precipitation' in filename or 'wind' in filename:
                            climate_plots.append(image_path)
                        elif 'disaster' in filename or 'hazard' in filename:
                            disaster_plots.append(image_path)
                        elif 'population' in filename:
                            population_plots.append(image_path)
                        else:
                            other_plots.append(image_path)

                    # Display climate comparison plots
                    if climate_plots:
                        st.markdown("**Climate Data:**")
                        for image_path in climate_plots:
                            st.image(image_path)

                    # Display disaster/hazard plots
                    if disaster_plots:
                        st.markdown("**Natural Hazards:**")
                        for image_path in disaster_plots:
                            st.image(image_path)

                    # Display population plots
                    if population_plots:
                        st.markdown("**Population Data:**")
                        for image_path in population_plots:
                            st.image(image_path)

                    # Display other analysis plots (from python_repl)
                    if other_plots:
